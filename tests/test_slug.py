...
def sample_model() -> Iterator[Any]:
    ...
    inst = Model()
    inst.query = MagicMock()  # type: ignore[misc]
    yield inst

    # Remove table from declarative base
    Base.metadata.remove(cast("Table", Model.__table__))
    Base.registry.dispose()


def test_set_slug_is_slug_no_name(sample_model: Any) -> None:
...
def test_set_slug_is_slug_is_name(sample_model: Any) -> None:
...
def test_set_slug_no_slug_no_name(sample_model: Any) -> None:
...
def test_set_slug_no_slug_is_name(sample_model: Any) -> None:
...
def test_set_slug_slug_unique(sample_model: Any) -> None:
...
def test_set_slug_truncate(sample_model: Any) -> None:
